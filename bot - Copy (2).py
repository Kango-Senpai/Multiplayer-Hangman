...
async def update(guess,ctx):
    if len(guess.strip()) > 1:
        ...
        await ctx.channel.send("Invalid guess")
    elif guess in newGame.guessedChars:
        await ctx.channel.send("Duplicate guess \'%s\'" % guess)
        ...
        if guess in newGame.requiredChars:
            await ctx.channel.send(guess + " is correct")
            newGame.readyPlayer = next(players,newGame.readyPlayer)
            #await ctx.channel.send("========================")
        else:
            await ctx.channel.send(guess + " is incorrect")
            #await ctx.channel.send("========================")
            newGame.readyPlayer = next(players,newGame.readyPlayer)
            newGame.guesses -= 1
